...
function sideMenu_initEvent() {
  document
    .getElementById('btn-sideOnOff')
    .addEventListener('click', sideMenu_clickSideOnoffBtn);
}
...
function sideMenu_clickSideOnoffBtn(e) {
  if (sideMenuObj.isSideOn) {
    // on으로 바꿔준다.
    document
      .getElementById('btn-sideOnOffIcon')
      .classList.add('fa-angle-double-left');
    document
      .getElementById('btn-sideOnOffIcon')
    ...
    for (let i = 0; i < sideMenuObj.content.childElements.length; i++) {
      sideMenuObj.content.childElements[i].style.transition =
